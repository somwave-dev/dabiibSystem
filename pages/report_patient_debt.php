...
if ((string) ($_GET['export'] ?? '') === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="patient-debt-' . date('Y-m-d') . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['Patient ID', 'Patient', 'Phone', 'Type', 'Guarantor', 'Credit Limit', 'Balance', 'Total Paid', 'Last Payment']);
...
clinic_reports_page_shell_start('Patient Debt', 'Outstanding balances, filters, and quick collection shortcuts.');

$queryParamsDebt = array_filter([
    'q' => $search,
    'type' => $typeFilter,
    'min_balance' => $minBalance > 0 ? $minBalance : '',
], static fn (mixed $v) => $v !== '' && $v !== null);
$csvDebtQs = http_build_query($queryParamsDebt + ['export' => 'csv']);
?>
<div class="report-actions-bar mb-4">
    ...
    <div class="report-actions-push">
        <a class="btn btn-primary btn-sm" href="report_patient_debt.php?<?php echo clinic_h($csvDebtQs); ?>"><i class="ti ti-file-spreadsheet me-1"></i>Download CSV</a>
...
<div class="card report-filter-sheet mb-4">
    <div class="card-body">
        <form class="row g-3 align-items-end" method="get" action="report_patient_debt.php">
            <div class="col-md-5">
            ...
            <div class="col-auto d-flex gap-2">
                ...
                <a class="btn btn-outline-secondary" href="report_patient_debt.php">Reset</a>
            </div>
        </form>
    </div>
</div>
...
<div class="row g-3 mb-4">
    <?php clinic_metric_card('Patients owing', $patientsOwing, 'ti-users', 'danger', 'After filters'); ?>
    <?php clinic_metric_card('Total debt', clinic_money($totalDebt), 'ti-cash-banknote', 'warning', 'Displayed rows'); ?>
    <?php clinic_metric_card('Over credit limit', $overCreditLimit, 'ti-alert-triangle', 'primary', ''); ?>
    <?php clinic_metric_card('Average debt', clinic_money($averageDebt), 'ti-chart-dots', 'success', 'Per row'); ?>
</div>
...
<div class="card report-data-card">
    ...
    <div class="card-body table-responsive">
        ...
        <?php if ($filteredPatients === []): ?>
            <div class="alert alert-light border text-center mb-0">No patients match these filters.</div>
        <?php endif; ?>
    </div>
</div>
