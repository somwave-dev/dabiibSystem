...
function clinic_sp_exec(string $procedure, array $params = [], ?string $types = null): int
{
    global $conn;
    ...
    // LAST_INSERT_ID() is session-scoped, so it survives the CALL flush and
    // reports the id generated by an INSERT performed inside the procedure.
    $res = $conn->query('SELECT LAST_INSERT_ID() AS id');

    return (int) (($res ? $res->fetch_assoc() : null)['id'] ?? 0);
}
