...
// Look up the account carrying this activation token.
$co = new Codes();
$db = $co->db;
$stmt = $db->prepare(
    'SELECT User_ID, Username, email, `status`, activation_token, activation_expires_at '
    . 'FROM users WHERE activation_token = ? AND deleted = 0 LIMIT 1'
);
...
$expiresAt = (string) ($user['activation_expires_at'] ?? '');
$expired = $expiresAt === '' || strtotime($expiresAt) < time();

if ((string) ($user['status'] ?? '') === 'active') {
    $db->close();
    header('Location: login.php?activation=already', true, 302);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($password) < 6) {
        ...
        $up = $db->prepare("UPDATE users SET Password_Hash = ?, status = 'active', activation_token = NULL, activation_expires_at = NULL WHERE User_ID = ? AND activation_token = ?");
        $up->bind_param('sis', $hash, $userId, $token);
        $up->execute();
        $up->close();
        $db->close();

        clinic_audit_record('Account activated', 'Account activated and password set for user ' . (string) ($user['Username'] ?? ''), 'user', $userId);
        clinic_notify_record('Account activated', 'Your account has been activated and your password is now set.', 'success', 'pages/profile.php', $userId);

        // Sign the current browser session out so the user lands on the login
        // page and can sign in as the newly activated account.
        $_SESSION = [];
        if (session_status() === PHP_SESSION_ACTIVE) {
            session_destroy();
        }

        header('Location: login.php?activated=1', true, 303);
        exit;
    }
}
...
$pageTitle = 'Activate Account | ' . $site['name'];
require_once __DIR__ . '/includes/auth_head.php';
?>
			<div class="text-center mb-4">
				<h4 class="mb-1 fw-bold">Activate Your Account</h4>
			...
			<?php endif; ?>
			<form method="post" action="activate-account.php?token=<?php echo urlencode($token); ?>" id="activateForm" novalidate autocomplete="off">
				...
				<button type="submit" class="btn btn-primary text-white w-100 py-2 mb-0">Activate &amp; Set Password</button>
			</form>
